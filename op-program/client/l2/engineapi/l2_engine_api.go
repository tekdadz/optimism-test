...
func (ea *L2EngineAPI) forkchoiceUpdated(_ context.Context, state *eth.ForkchoiceState, attr *eth.PayloadAttributes) (*eth.ForkchoiceUpdatedResult, error) {
	...
	if state.FinalizedBlockHash != (common.Hash{}) {
		...
		if finalHeader == nil {
			...
			ea.log.Warn("Final block not in canonical chain", "number", block.NumberU64(), "hash", state.HeadBlockHash, "canonicalhash", ea.backend.GetCanonicalHash(finalHeader.Number.Uint64()), "finalhash", state.FinalizedBlockHash)
